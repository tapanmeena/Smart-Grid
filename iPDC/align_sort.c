...
/*		}*/
		...
		} else if((old_df_front != -1) &&  ((df_soc < global_df_soc) || ((df_soc == global_df_soc) && (df_fracsec < global_df_fracsec)))) {
...
void assign_df_to_TSB(struct data_frame *df,int index) { 
	...
	if(TSB[index].soc == NULL) { // 1 if
		...
		if(old_df_front == -1) {
			...
			unsigned int soc1,fsec1;
			soc1 = to_long_int_convertor(TSB[index].soc);
			fsec1 = to_long_int_convertor1(fsec);

			global_df_soc = soc1;
			global_df_fracsec = fsec1;	
			ODFT[old_df_rear].soc = soc1;
			ODFT[old_df_rear].fracsec = fsec1;
...
//		TSB[index].count = 1;
		...
		/********************************* Create thread for setting timer ***********************************/
		...
		if(TSB[index].count == TSB[index].num) {
				...
				int xx = index;			
					pthread_cancel(TSB[xx].threadID);
				//printf("count full intermediate_dispatch 1\n");	
		...
		if(TSB[index].first_data_frame == NULL) { // 2 if 
			...
			if(TSB[index].count == TSB[index].num) {
				...
				int xx = index;			
				pthread_cancel(TSB[xx].threadID);
//				printf("count full timer intermediate_dispatch 2\n");
				intermediate_dispatch(xx);
			}
			...
			if(TSB[index].count == TSB[index].num) {
				...
				pthread_cancel(TSB[xx].threadID);
				//printf("index %d\n",xx);				
//				printf("count full timer intermediate_dispatch 3\n");
				intermediate_dispatch(xx);
			}
		} // 2 if ends	
...
/*printf("Dispatch\n");*/
		...
		Analysing(11,tm.tv_sec,tm.tv_usec,index);
		...
		if(IamSPDC == true) {
			...
			/*if(angleDiffApp == true) {
				...
				pthread_t t;
				if((err = pthread_create(&t,&attr,VoltageStability,(void *)&m))) {
					perror(strerror(err));		     
					exit(1);
				}
			}
			...
			if(coherencyOfGen == true) {
				...
				pthread_t t;
				if((err = pthread_create(&t,&attr,CoherencyOfGenerators,(void *) &m))) {
					perror(strerror(err));		     
					exit(1);
				}
			}
			...
			if(stateEstimation == true) {
				...
				pthread_t t;
				if((err = pthread_create(&t,&attr,stateEstimator,(void *)&m))) {
					perror(strerror(err));		     
					exit(1);
				}
			}	*/					
...
void checkForMissingFrames(ENTRY *found) {
	...
	if(((struct PMUInfo *)found->data)->visited == true) {
		...
		//printf("Id Code of Missing frame %d\n", ((struct PMUInfo *)found->data)->Idcode);
		dropCount++;
		free(temp);			
...
void *startTimer(void *ind) {
	...
	long index;
	index = (long)ind;
	TSB[index].threadID = pthread_self();
	usleep(waitTime);	
	pthread_mutex_lock(&mutex_on_TSB);			
	intermediate_dispatch(index);
...
int matchDataFrameTimeToTSBTime(unsigned int df_soc,unsigned int df_fracsec) {
	...
	else
	{
		...
		return xxx; // Find ned TSB where the frame would be assigned
	}	 	
}
